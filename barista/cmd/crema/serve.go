...
The socket path defaults to /tmp/crema-<pid>.sock. The shell eval line is
printed to stdout so you can do: eval $(crema serve)`,
	RunE: runServe,
}
...
func init() {
	serveCmd.Flags().StringVar(
		&socketFlag, "socket", "",
		"Unix socket path (default: /tmp/crema-<pid>.sock)",
	)
}
...
func runServe(cmd *cobra.Command, args []string) error {
	...
	if socketPath == "" {
		socketPath = fmt.Sprintf("/tmp/crema-%d.sock", os.Getpid())
	}
